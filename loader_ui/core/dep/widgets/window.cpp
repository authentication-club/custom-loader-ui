...
bool scrollbar_ex(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_visible_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    // Render
    draw->rect_filled(window->DrawList, bb.Min, bb.Max, draw->get_clr(clr->main.text, 0.04 * state->alpha * g.Style.Alpha), var->style.scrollbar_rounding);
    ImRect grab_rect;
    if (axis == ImGuiAxis_X)
        grab_rect = ImRect(ImLerp(bb.Min.x, bb.Max.x, grab_v_norm), bb.Min.y, ImLerp(bb.Min.x, bb.Max.x, grab_v_norm) + grab_h_pixels, bb.Max.y);
    else
        grab_rect = ImRect(bb.Min.x, state->y_offset, bb.Max.x, state->y_offset + grab_h_pixels);
    draw->rect_filled(window->DrawList, grab_rect.Min, grab_rect.Max, draw->get_clr(clr->main.accent, state->alpha * g.Style.Alpha), var->style.scrollbar_rounding);
...
bool c_gui::begin(std::string_view name, bool* p_open, window_flags flags)
{
    ...
    if (first_begin_of_the_frame && !window->SkipRefresh)
    {
        ...
        scroll_state* state = gui->anim_container<scroll_state>(id);

        float needed_scroll_clamp = calc_next_scroll_from_scroll_target_and_clamp(window, true).y;

        gui->easing(state->scroll, needed_scroll_clamp, 16.f, dynamic_easing);
        ...
        if (g.ActiveId != GetWindowScrollbarID(window, ImGuiAxis_Y))
        {
            if (!(flags & ImGuiWindowFlags_NoScrollWithMouse) && window->Scroll.y != needed_scroll_clamp)
                window->Scroll.y = std::round(state->scroll);
        }
        else
        {
