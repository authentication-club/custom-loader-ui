...
bool c_widgets::text_field(std::string_view widgets_id, std::string_view name, std::string_view icon, char* buf, int size)
{
    ...
    gui->easing(state->icon, hasContent || state->value_changed ? clr->main.accent.Value : clr->main.text.Value, 16.f, dynamic_easing);

    draw->rect_filled(window->DrawList, total.Min, total.Max, draw->get_clr(clr->main.text, 0.02 + state->alpha[0]), SCALE(elements->widgets.rounding));
