...
void c_gui::move_window(HWND hwnd, RECT rc)
{
    ...
    const bool auth_stage = (var->gui.stage_count == 0);
    const ImVec2 drag_extent = auth_stage
        ? SCALE(ImVec2(var->window.default_size.x, elements->top_bar.height + elements->window.padding.y))
        : SCALE(ImVec2(var->window.new_width, elements->top_bar.height + elements->window.padding.y));

    if (ImRect{ gui->window_pos(), gui->window_pos() + drag_extent }.Contains(GImGui->IO.MousePos) && gui->mouse_clicked(mouse_button_left))
        var->gui.dragging = true;

    if (!gui->mouse_down(mouse_button_left))
    {
        if (var->gui.dragging)
            ImGui::ResetMouseDragDelta(mouse_button_left);
    ...
    if (auth_stage)
    {
        if (var->gui.dragging)
        {
            ...
            const ImVec2 delta = GetMouseDragDelta();
            SetWindowPos(
                hwnd,
                nullptr,
    ...
    if (var->gui.update_size || var->gui.dragging)
    {
        GetWindowRect(hwnd, &rc);
        const ImVec2 delta = GetMouseDragDelta();
        const ImVec2 target_size = SCALE(var->window.size);
...
{
    std::string translated_text = var->gui.lang_count == 0 ? std::string(eng) : std::string(rus);
    const ImGuiID id = gui->get_window()->GetID(var->gui.lang_count == 0 ? eng.data() : rus.data());
...
void c_gui::loading()
{
    ...
    {
        float alpha[4]{ 0, 0.f, 0.24, 0.06f };
        float radius{ SCALE(elements->loading.radius) };
        float rotate{ 0 };
        float padding{ SCALE(elements->window.padding.x / 2 + elements->loading.radius) };
        float injecting_timer{ 1.f };
        float timer{ 0.f };
        bool loaded{ false };
        float offset{ 0 };
        float win_alpha[2]{ 0, 0 };
        bool clicked{ false };
    ...
    gui->easing(state->offset, state->loaded ? 0 : SCALE(elements->window.padding.y + elements->loading.notify_height), 12.f, dynamic_easing);

    ImRect notify_rect(gui->window_pos() + ImVec2(SCALE(elements->window.padding.x), gui->window_height() - SCALE(elements->window.padding.y + elements->loading.notify_height) + state->offset), gui->window_pos() + ImVec2(gui->window_width() - SCALE(elements->window.padding.x), gui->window_height() - SCALE(elements->window.padding.y) + state->offset));
    const ImRect button_rect(ImVec2(notify_rect.Max.x - SCALE(elements->window.padding.x + elements->loading.button_padding * 2) - gui->text_size(font->get(suisse_intl_semi_bold_data, 16), gui->language("Close window", "Закрыть окно").data()).x, notify_rect.Min.y + SCALE(elements->window.padding.y)), ImVec2(notify_rect.Max.x - SCALE(elements->window.padding.x), notify_rect.Max.y - SCALE(elements->window.padding.y)));

    bool is_loading = var->gui.loading;

    if (is_loading && !state->was_loading) {
        state->loaded = false;
        state->injecting_timer = 1.f;
        state->timer = 0.f;
        state->rotate = 0.f;
    }
    ...
    if (is_loading)
    {
        ...
        if (state->timer > 2.f && state->rotate <= -1.998f)
        {
            state->timer = 0;
            state->rotate = 0.f;
        }

        gui->easing(state->radius, (state->timer < 1.2f) ? SCALE(elements->loading.radius * 1.5) : SCALE(elements->loading.radius), 10.f, dynamic_easing, true);
        gui->easing(state->padding, (state->timer < 1.2f) ? 0 : SCALE(elements->window.padding.x / 2 + elements->loading.radius), 12.f, dynamic_easing, true);

        if (state->rotate >= -1.998f)
            state->rotate += gui->fixed_speed(-1.3f);

        draw->rotate_start(drawlist);
        draw->circle_filled(drawlist, pos + size / 2 - ImVec2(state->padding, -(state->padding)), state->radius, draw->get_clr(clr->main.accent, 1 - state->alpha[0]), 60);
        draw->circle_filled(drawlist, pos + size / 2 + ImVec2(state->padding, -(state->padding)), state->radius, draw->get_clr(clr->main.accent, 0.4 * (1 - state->alpha[0])), 60);
        draw->circle_filled(drawlist, pos + size / 2 + ImVec2(state->padding, state->padding), state->radius, draw->get_clr(clr->main.accent, 0.6 * (1 - state->alpha[0])), 60);
        draw->circle_filled(drawlist, pos + size / 2 - ImVec2(state->padding, state->padding), state->radius, draw->get_clr(clr->main.accent, 0.8 * (1 - state->alpha[0])), 60);
        draw->rotate_end(drawlist, 360 * state->rotate, pos + size / 2);

    }

    gui->easing(state->alpha[0], state->loaded ? 1.f : 0.f, 8.f, static_easing);

    if (button_rect.Contains(GImGui->IO.MousePos) && gui->mouse_clicked(mouse_button_left))
        state->clicked = true;
    if (state->alpha[1] >= 0.99)
    {
        state->clicked = false;
        if (!is_loading) {
            state->injecting_timer = 1.f;
    ...
    bool hovered = button_rect.Contains(GImGui->IO.MousePos);

    gui->easing(state->alpha[1], state->clicked ? 1.f : (hovered ? 0.1f : 0.f), 8.f, static_easing);
    gui->easing(state->alpha[2], state->clicked ? 1.f : (hovered ? 0.5f : 0.24f), 8.f, static_easing);
    gui->easing(state->alpha[3], state->clicked ? 0.48f : (hovered ? 0.2f : 0.06f), 8.f, static_easing);


    gui->push_var(style_var_alpha, state->alpha[0]);
    draw->rect_filled(drawlist, notify_rect.Min, notify_rect.Max, draw->get_clr(clr->loading.win_bg), SCALE(elements->widgets.rounding));
    draw->rect(drawlist, notify_rect.Min, notify_rect.Max, draw->get_clr(clr->main.text, 0.04), SCALE(elements->widgets.rounding));

    draw->text_clipped(drawlist, font->get(suisse_intl_semi_bold_data, 16), notify_rect.Min + SCALE(elements->window.padding) - SCALE(0, 2), notify_rect.Max, draw->get_clr(clr->main.text), gui->language("Session completed successfully!", "Сессия завершена успешно!").data());
    
    draw->text_clipped(drawlist, font->get(suisse_intl_medium_data, 16), notify_rect.Min + SCALE(elements->window.padding.x, 0), notify_rect.Max - SCALE(0, elements->window.padding.y - 2), draw->get_clr(clr->main.text, 0.48), gui->language("Close that window.", "Закройте это окно.").data(), NULL, NULL, ImVec2(0.f, 1.f));
    
    draw->rect_filled(drawlist, button_rect.Min, button_rect.Max, draw->get_clr(clr->main.text, 0.04), SCALE(elements->widgets.rounding));
    draw->rect_filled(drawlist, button_rect.Min, button_rect.Max, draw->get_clr(clr->main.accent, state->alpha[1]), SCALE(elements->widgets.rounding));
    draw->text_clipped(drawlist, font->get(suisse_intl_semi_bold_data, 16), button_rect.Min, button_rect.Max, draw->get_clr(clr->main.text, state->alpha[2]), gui->language("Close window", "Закрыть окно").data(), NULL, NULL, ImVec2(0.5f, 0.5f));
    draw->rect(drawlist, button_rect.Min, button_rect.Max, draw->get_clr(clr->main.text, state->alpha[3]), SCALE(elements->widgets.rounding));
    gui->pop_var();
